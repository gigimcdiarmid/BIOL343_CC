...
positpal::start("Group_4_CC2.Rmd")
```
...
1. **Gigi McDiarmid** — 20385910
    + [BIOL343_CC](https://github.com/gigimcdiarmid/BIOL343_CC)
1. 
...
---------|-------------|------
population_type | reproduction type | asexual or sexual 
ovary_width | measured flower ovary width | mm
ovary_height | measured flower ovary height| mm
flower_number | number of flowers on plant | count
...
cc1_data <- cc1_data[1:46, c("X.population_type", "ovary_width", "ovary_height","flower_number")]
...
$$ OvaryWidth \times OvaryHeight = OvaryArea$$
Ovary Area is measured in $mm^2$ 
...
area_means <- aggregate(cc1_data$ovary_area,list(cc1_data$X.population_type), mean)
...
```{r area-histogram, echo=F, message=F}
library(ggplot2)

ggplot(cc1_data, aes(x=ovary_area)) +
    geom_histogram(binwidth=1) + 
    facet_wrap(.~X.population_type) +
    labs(title = "Ovary Area by Population Type", x ="Ovary Area (mm^2)", y = "Count") +
    theme_classic()
```
...
Source: [BIOL343 OnQ](https://onq.queensu.ca/d2l/le/dropbox/1208667/492096/DownloadAttachment?fid=54136518)
